...
## a particle like this could effect every thing in the whole universe ...
...
###           that make up a particle. 
 ...
 $cz= 2**(-2**(-1**$t*$c));  ###    $t*$c; 
 $cy= 2**(-2**(-1**$t*sin($t*$c)));  ###   $t*sin($t*$c); 
 $cx= 2**(-2**(-1**$c*bessj0(rvals($t,{Centre=>[0]}))*$c));  ### $c*bessj0(rvals($t,{Centre=>[0]}))*$c;
